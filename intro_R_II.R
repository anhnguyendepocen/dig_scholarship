...
head(mtcars) #look at the first 6 rows of the Motor Trend Car Road Tests data frame

?mtcars #look at documentation for this data
...
hist(mtcars$mpg, breaks=20) #define splits with 'breaks = '


# Create histograms of horsepower (hp) and 1/4 mile time (qsec) with different splits
#
#
...
hist(mtcars$mpg, 
     ...
     main = 'Histogram of MPG', #title (note the quotation marks)
...
# NOTE: The above is analogous to this:
...
summary(lin_mod) #check model output for significance, r-squared, equation of line (y = mx + b)
...
# Create a scatterplot with 2 numeric continuous variables in mtcars that likely have a relationship
#
#
...
# HINT: Need to create a new linear model object to reflect your plotted relationship
#
#
...
par(cex = 2, #changes size of plotting text and symbols
    ...
    cex.axis = 1.3, #changes axis text size
    
    cex.lab = 1.4, #changes label text size
    
    cex.main = 1.5, #changes text title size
    
    col = 'grey', #changes axes color
    
    col.axis = 'grey', #changes axes text colors
    
    col.lab = 'grey', #changes x and y label colors
    
    col.main = 'darkblue', #changes title color
    
    bg = 'black') #changes background color

# NOTE: The default axis and label size in R is too small for publication figures.


# Run your last scatterplot (plot()) and notice how it has changed according to the par settings
#
#
...
# Re-run 4 of your plots above and notice how they'll create a panel
#
#


dev.off() #return plotting parameters to default again
...
# Run (i.e. create) one of your plots
...
setwd('~/Desktop/') #Mac: set working directory to Desktop folder

setwd('c:/Users/.../Desktop/') #Windows: replace ... with your username


pdf('mpgVcyl.pdf') #function AND file extension indicates in what format to save plot in
...
jpeg('fig1.jpeg') #saves plot as JPEG

postscript('fig1.eps') #saves plot as EPS
...
# I. Trellis graphs: display relationships between variables conditioned on one or more variables
...
xyplot(mpg ~ cyl | am, groups = carb, data = mtcars) #additionally split with groups =

# Plot effect of cylinder on MPG, split by carburetor and with transmission in different colors
#
#
...
# Choose any one of your lattice plots and:
...
ggplot(data = mtcars, aes(x = hp, y = mpg)) #creates plotting space with variables
...
ggplot(data = mtcars, aes(x = hp, y = mpg)) + #plus symbol at end of line for new layer below
  
  geom_point() #plot data as points (see ggplot2 Cheat Sheet at bottom of script for others)
...
# For efficiency, create an object that is your foundational plotting space

myplot <- ggplot(data = mtcars, aes(x = hp, y = mpg))
...
# NOTE: If we had created our plot object with geom_point, geom_point could not be modified.
...
myplot + #return to myplot, which does not yet have geom_point
...
# Create an object myplot4 with myplot3 and this new theme
#
#
...
#NOTE: Need to set your working directory again if changed or re-started R

setwd('~/Desktop/') #Mac: set working directory to Desktop
